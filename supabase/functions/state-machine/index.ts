...
serve(async (request: Request): Promise<Response> => {
  try {
    ...
    return jsonResponse(
      {
        error:
          "Action invalide. Utilisez create_transaction, transition_status, initiate_deposit, initiate_payout, initiate_refund, generate_pin, validate_pin, set_user_suspension ou set_requires_human.",
      },
      400,
    );
  } catch (error) {
